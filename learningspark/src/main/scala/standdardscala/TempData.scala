...
object TempData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // groupBy returns a Map[Int, Array[TempData]]
    // groupBy se 12 key hain aur unkey values Array[TempData] hain
    // Array[TempData] mein har element ek month ka data hain aur har element mein har day ka data hai 
    val monthlyTemp = monthGroups.map { case (m, days) =>
      m -> days.foldLeft(0.0)((sum, td) => sum+td.tmax)/days.length
    }
    // sum me 0 hai, td is days. td.tmax is tmax of days. sum+td.tmax is sum of tmax of days. 
